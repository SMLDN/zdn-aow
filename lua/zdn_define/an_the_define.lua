TASK_LIST = {
    ["newschool_xingmiao"] = {
        {"tmc_tmtc", "Trầm Mặc Thả Câu", "zdn_logic_an_the_tmc_tmtc"},
        {"tmc_tthd", "Tuần Tra Hòn Đảo", "zdn_logic_an_the_tmc_tthd"},
        {"tmc_ctnk", "Cốc Trung Ngộ Kiếm", "zdn_logic_an_the_tmc_ctnk"},
        {"tmc_qtdlk", "Quan Tinh Đài Luyện Kiếm", "zdn_logic_an_the_tmc_qtdlk"},
        {"tmc_tvmp", "Tạp Vụ Môn Phái", "zdn_logic_an_the_tmc_tvmp"}
    },
    ["newschool_nianluo"] = {
        {"nlb_tlph", "Thí luyện Phấn Hoa", "zdn_logic_an_the_nlb_tlph"},
        {"nlb_thnp", "Tặng hoa nhận phấn", "zdn_logic_an_the_nlb_thnp"}
        -- {"nlb_tldn", "Tù Lung Đấu Nô", "zdn_logic_an_the_nlb_tldn"},
        -- {"nlb_tvk", "Tập Vũ Kỹ", "zdn_logic_an_the_nlb_tvk"}
    }
}
